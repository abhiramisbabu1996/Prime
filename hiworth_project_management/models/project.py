...
class TaskWizard(models.Model):
    ...
    def task_confirm(self):
        ...
        return {
            ...
            'domain': ['|', ('project_manager', '=', self.assigned.id),
                       ('user_id', '=', self.assigned.id)] if self.assigned.id else [],
            'target': 'current',
            'type': 'ir.actions.act_window',
            'context': {'default_user_id': self.assigned.id, 'default_direct_task': direct_task,
                        'default_assigned': [(6, 0, [self.assigned.id])],
                        'default_project_id': self.env.context.get('default_project_id'),
                        'default_wd_id': self.env.context.get('default_wd_id'),
                        'default_activity': self.env.context.get('default_activity'),
                        'default_site_visit': self.env.context.get('default_site_visit')}
        }
...
class TaskWizardLine(models.Model):
    ...
    def create(self, vals):
        ...
        for i in vals['assigned'][0][2]:
            ...
            if usr:
                emp = self.env['hr.holidays'].search(
                    [('employee_id', '=', usr.employee_id.id), ('state', '=', 'validate')])
                if emp:
                    for j in emp:
...
class EvaluateRating(models.Model):
    ...
    def return_rating(self):
        ...
        res = {
            ...
            'view_id': view_id,
            'target': 'new',
            'context': {'default_rec': self.rec.id,
                        'default_rate_to': self.rec.user_id.id if self.env.user.id != 1 else self.rec.project_manager.id,
                        'default_flag': 'flag', 'default_report_rate': False}
        }
    ...
    def proceed_rating(self):
        ...
        res = {
            ...
            'context': {'default_rec': self.rec.id,
                        'default_rate_to': self.rec.user_id.id if self.env.user.id != 1 else self.rec.project_manager.id,
                        'default_flag': 'draft', 'default_report_rate': False}
        }
        return res
    ...
    def rate_staff(self):
        ...
        if self.rec.emp_rate == True and self.rec.man_rate == False:
            ...
            self.rec.priority_man = self.task_rating
            if self.report_rate == False and self.flag == 'flag':
                view_ref = self.env['ir.model.data'].get_object_reference('hiworth_project_management',
                                                                          'event_all_tasks_form')
                view_id = view_ref[1] if view_ref else False
                res = {
                    'type': 'ir.actions.act_window',
                    'name': _('Task'),
...
class EventEvent(models.Model):
    ...
    def onchange_date_end(self):
        ...
        if self.date_end:
            if self.date_end < self.date_begin:
                ...
                events = self.env['event.event'].search(
                    [('user_id', '=', self.user_id.id), '|', '&', ('date_begin', '>=', self.date_begin),
                     ('date_begin', '<=', self.date_end), '&', ('date_end', '>=', self.date_begin),
    ...
    def finish_task(self):
        ...
        for g in self.env['res.groups'].search(
                [('sequence', '<', self.user_id.employee_id.user_category.sequence), ('sequence', '>', 0)]):
            for user in g.users:
                if user.id not in user_ids:
                    user_ids.append(user.id)
        for u in user_ids:
    ...
    status = fields.Selection([('initial', 'Not Completed'), ('completed', 'Completed')], default="initial",
                              string="Status")
    remarks = fields.Text('Remarks')
    ...
    def button_evaluate(self):
        if self.emp_rate == True:
            ...
            res = {
                ...
                'target': 'new',
                'context': {'default_rec': self.id,
                            'default_rate_to': self.user_id.id if self.env.user.id != 1 else self.project_manager.id,
                            'default_flag': 'flag' if flag == 1 else 'draft'}
            }
    ...
    def send_work_report(self):
        ...
        res = {
            ...
            'view_id': view_id,
            'target': 'current',
            'context': {'default_req_end': True if self.state == 'completed' else False, 'default_report': self.report,
                        'default_project': self.project_id.id, 'default_task_id': self.id,
                        'default_date_end': self.date_end, 'default_branch_id': self.user_id.company_id.id}
        }
...
class ProjectProject(models.Model):
    ...
    def create(self, vals):
        ...
        if result['name'] == '/':
            ...
            result['name'] = str('PBA') + str('/') + str(result['company_id'].name) + str('/') + str(
                result['project_category'].name) + str('/') + seq[:3] + str('/') + seq[-4:]
        job_assign = self.env['job.assignment']
        dob = ''
        if result['partner_id'].years:
            dob = datetime.strptime(str(result['partner_id'].years) + "-" + str(result['partner_id'].dobm) + "-" + str(
                result['partner_id'].days), "%Y-%B-%d").strftime("%Y-%m-%d")
    ...
    status_pro = fields.Selection([('ongoing', 'On Going'), ('pending', 'Pending'), ('completed', 'Completed')],
                                  default='ongoing')
        ...
        if self.company_id:
            ...
            for usr in user:
                if self.company_id == usr.employee_id.company_id:
                    if (
                    self.pool['res.users'].has_group(self._cr, self._uid, 'project.group_project_manager') or self.pool[
                        'res.users'].has_group(self._cr, self._uid, 'hiworth_project_management.group_general_manager'),
                    self.pool['res.users'].has_group(self._cr, self._uid,
                                                     'hiworth_project_management.group_general_manager')):
                        users.append(usr.id)
...
class ResPartner(models.Model):
    ...
    is_a_client = fields.Boolean('Customer/Client ?')
    ...
    def onchange_country_id_pro(self):
        if self.country_id or self.country_id1:
            ...
            record = self.env['res.country.state'].search(
                [('country_id', '=', self.env['res.country'].search([('code', '=', 'IN')])[0].id)])
...
class JobAssignment(models.Model):
    ...
    def convert_to_project(self):
        ...
        view_ref = self.env['ir.model.data'].get_object_reference('hiworth_project_management',
                                                                  'view_project_manager_charge_form')
        view_id = view_ref[1] if view_ref else False
        res = {
            'type': 'ir.actions.act_window',
...
class ManagerCharge(models.Model):
    ...
    def confirm_manager(self):
        ...
        group_id_general_manager = self.env['ir.model.data'].get_object('hiworth_project_management',
                                                                        'group_general_manager').id
